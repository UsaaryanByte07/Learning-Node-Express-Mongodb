...
const getHostHomes = async (req, res, next) => {
  try {
    ...
    res.render("host/host-homes", { addedHomes, pageTitle: "Host Houses", isLoggedIn: req.session.isLoggedIn});
  } catch (error) {
...
const postAddHome = async (req, res, next) => {
  try {
    ...
    await newHome.save();
    res.render("host/home-added", { pageTitle: "Home Added", isLoggedIn: req.session.isLoggedIn });
  } catch (error) {
